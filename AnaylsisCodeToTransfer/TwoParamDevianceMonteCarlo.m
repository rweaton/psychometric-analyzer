function DevianceDistributionStruct = TwoParamDevianceMonteCarlo(SOAs, nSamplesPerSOA, Params, nIterations, nBins)
    ...
    % of curve fit paramters 'EstimatedTwoParameterMonteCarlo' and each
    % value of 'Params' should be corresponding mean of each output
    % distribution for each parameter.
    
    nSOAs = length(SOAs);
    
    ModelVals = get_2parm_fit_vals(Params, SOAs);
    ...
    for j = 1:nIterations
        ...
        UnsortedDevianceVals(j,1) = ComputeDeviance2(SOAs, DataPoints, nSamplesPerSOA, Params);
