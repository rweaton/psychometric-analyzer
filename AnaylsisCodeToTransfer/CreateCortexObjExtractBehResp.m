function [ss1, tt1, uu1] = CreateCortexObjExtractBehResp(path, fname, unfoldgrp, unfoldedsoas, MaskTypeDelimitingConditions)
...
%(8) actual condition
%(9) stimulus duration
%%%% (10) Behavioral Response  [added to this version only]  %%%%%  
%(11) SOA (targeted SOA)
%(12) choice [0 if he picks A 1 if he picks B]
...
% Additional latency calculated here 
[PairToSacOnset, PairToSacOnsetT] = latency(fobj,robj,27,44);
saclat=NaN.*ones(length(pairsacT),1); %create empty list
saclat(ismember(pairsacT,PairToSacOnsetT))=PairToSacOnset;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% the latency list 'saclat' has been appended to array below as a column
codelist=[repmat(cname,ntrs,1),repmat(ii,ntrs,1),trs',samppair0',pairsac',cuelat,fobj.resps(trs),fobj.conds(trs),stimdurN,saclat];
%         [         1        ] [        2      ] [ 3] [     4  ] [  5   ] 
...
for ii=1:length(unfoldedsoas)
    seltrials1=ismember(codelist(:,11),unfoldedsoas(ii)); % correction made: 10 changed to 11 to account for appended column to codelist
...
for jj=1:length(latsgrp)
    ...
    seltrials1=seltrials1&ismember(codelist(:,11),ssoas); % correction made: 10 changed to 11 to account for appended column to codelist
    seltrials2=seltrials1;
...
ss1=codelist(seltrials1,11); % changed from 10 to 11 to account for appended column
tt1=codelist(seltrials1,12); % changed from 11 to 12 to account for appended column
uu1=codelist(seltrials1,10); % additional data that is outputed by this function

% filter out NaNs
tmp = (~isnan(tt1))&(~isnan(uu1));
ss1 = ss1(tmp);
tt1 = tt1(tmp);
uu1 = uu1(tmp);
...
% 
% for j = 1:length(tt1)
%    if  (~isnan(tt1(j,1)))&(~isnan(uu1(j,1)))
%       uu1_temp = [uu1_temp; uu1(j,1)];
