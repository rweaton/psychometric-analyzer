function UnsortedEstimatedParamVals = EstimatedTwoParameterMonteCarlo(EmpericalDataPoints, SOAs, nSamplesPerSOA, nIterations, nBins)
    ...
    % the two parameter logistic curve fitting routine so the ideal curve
    % fit parameters can be obtained.  Each set of curve fit parameters is
    % stored for each iteration.  Columns of the output array 
    % 'UnsortedEstimatedParamVals' represent each of the two parameters
    ...
    UnsortedEstimatedParamVals = NaN*ones([nIterations,2]);
    ...
    for j = 1:nIterations
        ...
        UnsortedEstimatedParamVals(j,:) = ...
            make_2par_logisticMC(SimulatedTrials(:,1),SimulatedTrials(:,2),SimulatedTrials(:,2),SOAs);
